...
@Component
public class AuthorityInQuery {
    @Autowired
    private MongoTemplate mongoTemplate;

    private PriorityBlockingQueue<HashMap<String,ArrayList<Authority>>> minHeap;

    public AuthorityInQuery() {
        minHeap = new PriorityBlockingQueue<HashMap<String,ArrayList<Authority>>>(11, new Comparator<HashMap<String,ArrayList<Authority>>>() { //大顶堆，容量11
            @Override
            public int compare(HashMap<String,ArrayList<Authority>> u1, HashMap<String,ArrayList<Authority>> u2) {
                return Integer.valueOf(u1.keySet().iterator().next()) - Integer.valueOf(u2.keySet().iterator().next());
            }
        });

    }
    public HashMap<String, ArrayList<Authority>>getlocat(int code){
        Iterator<HashMap<String,ArrayList<Authority>>> itor=minHeap.iterator();
        while(itor.hasNext() ){
    ...
    public void init(){
        ...
        queryIn.addCriteria(Criteria.where("behavior").is("in").where("status").is("receive"));
    ...
    public PriorityBlockingQueue<HashMap<String,ArrayList<Authority>>>getMinHeap(){return minHeap;}
    public void setMinHeap(PriorityBlockingQueue<HashMap<String,ArrayList<Authority>>>minHeap){this.minHeap=minHeap;}
